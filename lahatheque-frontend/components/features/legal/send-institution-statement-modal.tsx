...
import { Send, X, FileText, CheckCircle2, AlertCircle, Calendar, ShieldCheck, Building2, BookOpen } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import type { PeriodType } from "@/lib/types/legal";
import { sendInstitutionRoyaltyStatement } from "@/lib/services/legal";
...
interface SendInstitutionStatementModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
...
export function SendInstitutionStatementModal({
  isOpen,
  onClose,
  onSuccess,
  entity,
}: SendInstitutionStatementModalProps) {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      const res = await sendInstitutionRoyaltyStatement({
      ...
      if (res.success) {
        setSuccessMsg(res.message || "Bordereau partenaire expédié avec succès via le serveur officiel LAHAThèque.");
        setTimeout(() => {
          onSuccess?.();
          onClose();
        }, 1400);
      } else {
        setErrorMsg(res.error || "Impossible d'expédier le relevé partenaire.");
      }
    } catch (err: any) {
  ...
  const formattedAmount = entity.amount_due !== undefined
    ? new Intl.NumberFormat("fr-FR").format(entity.amount_due)
    : null;
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-serif font-bold text-navy text-base">
                  Relevé Institutionnel de Redevances
                </h2>
                <p className="text-xs text-foreground-muted">
                  Bordereau officiel adressé à l&apos;établissement partenaire
                </p>
              </div>
            </div>
            <button
