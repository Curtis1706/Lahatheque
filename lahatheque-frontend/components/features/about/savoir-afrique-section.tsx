...
import { Globe, ArrowRight, BookOpen, ZoomIn, ZoomOut, Sparkles } from "lucide-react";
...
export function SavoirAfriqueSection() {
  ...
  return (
    <section className="w-full max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 overflow-hidden">
      <div className="w-full space-y-8 sm:space-y-12">
        {/* En-tête de section universel (mobile + desktop) */}
        <div className="text-center max-w-3xl mx-auto space-y-2.5">
          <span className="inline-block text-xs font-bold tracking-[0.2em] uppercase text-gold">
            NOTRE VISION
          </span>
          <h2 className="font-serif text-2xl sm:text-3xl md:text-5xl font-bold tracking-tight text-navy leading-tight">
          ...
          <figure
            className="relative w-full max-w-6xl mx-auto border border-border rounded-2xl sm:rounded-3xl bg-background-secondary shadow-sm overflow-hidden mb-6 sm:mb-10"
          >
            ...
            <div className="flex sm:hidden items-center justify-between px-3.5 py-2 bg-background border-b border-border text-[11px] text-foreground-muted">
              <button
                type="button"
                onClick={() => {
                  setIsZoomedMobile(!isZoomedMobile);
                  setTimeout(() => {
                    if (figureRef.current && !isZoomedMobile) {
                      figureRef.current.scrollLeft = 180;
                    }
                  }, 50);
                }}
                className="flex items-center gap-1 font-bold text-navy hover:text-gold px-2 py-1 rounded bg-background-secondary border border-border transition-colors cursor-pointer"
              >
                {isZoomedMobile ? (
                  <>
            ...
            <div
              ref={figureRef}
              className={`relative w-full ${isZoomedMobile ? "overflow-x-auto scrollbar-thin" : "overflow-hidden"}`}
            >
              <div
                className={`relative transition-all duration-300 ${
                  isZoomedMobile ? "min-w-[760px]" : "w-full"
                }`}
                style={{ paddingBottom: isZoomedMobile ? "332px" : "43.75%" }}
              >
                <div
