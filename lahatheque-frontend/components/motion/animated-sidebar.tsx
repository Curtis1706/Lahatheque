"use client";
// beui.dev/components/motion/animated-sidebar adapté pour LAHAThèque
...
// Desktop morph transition with critically damped spring
  ...
  return (
    ...
    >
      <div
        ...
        className={cn(
          "group/sidebar-wrapper flex min-h-svh w-full min-w-0 bg-background",
          className
        )}
      >
        {children}
    ...
    >
      ...
      <motion.div
        ...
        className={cn(
          ...
          "border-navy-hover/40 bg-navy shadow-2xl will-change-transform text-white",
  ...
  function AnimatedSidebar(
    ...
    return (
      <motion.aside
        ...
        animate={{ width }}
        onMouseEnter={(event) => {
          if (expandOnHover && !context.isMobile) {
      ...
      >
        <motion.div
          ...
          transition={context.reduce ? REDUCED_TRANSITION : PANEL_TRANSITION}
          className={cn(
            "sticky top-0 flex h-svh w-full flex-col overflow-hidden bg-navy text-white shadow-lg",
            collapsible === "offcanvas" && "w-[var(--sidebar-width)]",
            variant === "sidebar" &&
              (side === "left"
                ? "border-navy-hover border-r"
                : "border-navy-hover border-l"),
            variant === "floating" &&
              "m-2 h-[calc(100svh-1rem)] rounded-2xl border border-navy-hover shadow-sm",
...
>(function AnimatedSidebarTrigger(
  ...
  return (
    <button
      ...
      type={type}
      aria-label={props["aria-label"] ?? "Basculer la barre latérale"}
      ...
      className={cn(
        "inline-flex size-10 shrink-0 items-center justify-center rounded-xl outline-none transition-colors",
        "hover:bg-navy-hover/50 text-white/80 hover:text-white",
...
>(function AnimatedSidebarClose(
  ...
  return (
    <button
      ...
      ref={forwardedRef}
      type={type}
      aria-label={props["aria-label"] ?? "Fermer la barre latérale"}
      onClick={(event) => {
        onClick?.(event);
      ...
      className={cn(
        "inline-flex size-9 shrink-0 items-center justify-center rounded-xl outline-none transition-colors",
        "text-white/70 hover:text-white hover:bg-navy-hover/50",
        "focus-visible:ring-2 focus-visible:ring-gold",
        className
      )}
    />
  ...
  return (
    <button
      ...
      className={cn(
        ...
        "after:absolute after:inset-y-0 after:left-1/2 after:w-px after:bg-transparent after:transition-colors hover:after:bg-gold/40",
...
>(function AnimatedSidebarHeader({ className, ...props }, forwardedRef) {
  return (
    <div
      ...
      className={cn("flex shrink-0 flex-col gap-2 p-3 border-b border-navy-hover/40", className)}
    />
  );
});
...
>(function AnimatedSidebarContent({ className, ...props }, forwardedRef) {
  return (
    <div
      ...
      className={cn(
        "flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto overflow-x-hidden overscroll-contain px-2.5 py-3",
...
>(function AnimatedSidebarFooter({ className, ...props }, forwardedRef) {
  return (
    <div
      ...
      className={cn(
        "flex shrink-0 flex-col gap-2 border-navy-hover/40 border-t p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]",
        className
      )}
    />
  );
...
>(function AnimatedSidebarGroupLabel(
  ...
  return (
    <div
      ...
      className={cn(
        "mb-1 h-6 overflow-hidden px-2 text-[10px] font-bold uppercase tracking-[0.14em] text-white/50 transition-opacity",
...
>(function AnimatedSidebarMenu(
  ...
  return (
    <SharedLayoutBg
      ...
      as="ul"
      inset={0}
      pillClassName="rounded-xl bg-navy-hover/60"
...
export function AnimatedSidebarMenuSubButton({
  ...
  const interactiveClassName = cn(
    "flex min-h-8 w-full min-w-0 items-center gap-2 rounded-xl px-2.5 text-left text-xs outline-none transition-colors",
    "text-white/70 hover:bg-navy-hover/50 hover:text-white",
    "focus-visible:ring-2 focus-visible:ring-gold",
    isActive && "bg-gold/10 text-gold font-bold",
...
export function AnimatedSidebarMenuButton({
  ...
  const content = (
    <>
      ...
      <motion.span
        ...
        className={cn(
          "relative z-10 min-w-0 flex-1 truncate text-xs font-semibold",
          isActive ? "text-gold font-bold" : "text-white/90",
      ...
      {badge && !panel.collapsed ? (
        <span className="relative z-10 shrink-0 text-[10px] px-1.5 py-0.5 rounded-full bg-gold/20 text-gold font-bold font-mono">
          {badge}
        </span>
      ) : null}
      {ariaExpanded !== undefined ? (
        <motion.span
          ...
          transition={context.reduce ? { duration: 0 } : SPRING_LAYOUT}
          className="relative z-10 grid size-4 shrink-0 place-items-center text-white/50"
        >
  ...
  const interactiveClassName = cn(
    "group relative flex min-h-10 w-full min-w-0 items-center gap-2.5 overflow-hidden rounded-xl text-left text-xs outline-none transition-colors cursor-pointer",
    panel.collapsed ? "justify-center px-0" : "px-3",
    "text-white/80 hover:text-white hover:bg-navy-hover/50",
    "focus-visible:ring-2 focus-visible:ring-gold",
