...
/**
 * @deprecated Ne plus utiliser ce tableau statique.
 * Les disciplines sont désormais chargées dynamiquement depuis la base de données
 * via `getDisciplines()` (lib/services/classification.ts) ou le hook `useDisciplines()`.
 * Ce tableau est conservé uniquement comme fallback de type pour la compatibilité TypeScript.
 */
export const ACADEMIC_DISCIPLINES: string[] = [
  /*appel à la bd à implémenter*/
