...
export default function AdminPublisherRoyaltiesPage() {
  ...
  useEffect(() => {
    getAdminRoyalties().then((items) =>
      setData(items.filter((i) => i.beneficiary_type === "publisher"))
    );
  }, []);
  ...
  const columns: DataTableColumn<AdminRoyalty>[] = [
    ...
    {
      key: "payout_amount",
      header: "Redevance Contractuelle",
      cell: (row) => (
      ...
      <div>
        ...
        <div className="pb-4 border-b border-border">
          <h1 className="text-xl sm:text-2xl font-bold font-serif text-navy">
            Redevances Éditeurs Tiers
          </h1>
          <p className="text-xs sm:text-sm text-foreground-muted mt-0.5">
            Suivi des reversements aux maisons d'édition partenaires selon les taux négociés par contrat.
          </p>
        </div>
      </div>
