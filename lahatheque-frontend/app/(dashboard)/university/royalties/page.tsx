                ...
                {data.bouquet_royalties.map((bouquet) => {
                  return (
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border pb-3">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all inline-flex items-center gap-1.5 cursor-pointer shadow-xs min-h-[34px] ${
                          ...
                          >
                            ...
                            <span>
                              {selectedBouquetForDistribution?.bouquet_id === bouquet.bouquet_id
                                ? "Masquer le camembert"
                                : "Voir le camembert"}
                            </span>
                            <ChevronDown
                              className={`w-3.5 h-3.5 transition-transform duration-200 ${
                                selectedBouquetForDistribution?.bouquet_id === bouquet.bouquet_id ? "rotate-180" : ""
                              }`}
                            />
                          </button>
                        </div>
                      ...
                      {/* Barre d'action explicite pour le camembert d'audience */}
                      <div className="pt-3 border-t border-border/70 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
                        <div className="flex items-center gap-2">
                          <span className="w-2 h-2 rounded-full bg-gold animate-pulse shrink-0" />
                          <p className="text-xs text-foreground-muted">
                            Consultez le camembert de répartition des consultations et le détail du prorata d&apos;audience de votre établissement.
                          </p>
                        </div>
                        <button
                          type="button"
                          ...
                          className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all inline-flex items-center justify-center gap-2 cursor-pointer shadow-xs min-h-[38px] shrink-0 ${
                            selectedBouquetForDistribution?.bouquet_id === bouquet.bouquet_id
                              ? "bg-gold text-navy font-bold ring-2 ring-gold/40 shadow-sm"
                              : "bg-navy text-white hover:bg-navy-hover"
                          }`}
                        >
                          ...
                          <span>
                            {selectedBouquetForDistribution?.bouquet_id === bouquet.bouquet_id
                              ? "Masquer le camembert d'audience"
                              : "Voir le camembert d'audience & redevances"}
                          </span>
                          <ChevronDown
                            className={`w-4 h-4 transition-transform duration-200 ${
                              selectedBouquetForDistribution?.bouquet_id === bouquet.bouquet_id ? "rotate-180 text-navy" : "text-white/80"
                            }`}
                          />
                        </button>
                      </div>

                      {/* Panneau de Répartition Intégré Directement DANS la carte */}
                      {selectedBouquetForDistribution?.bouquet_id === bouquet.bouquet_id && (
                          ...
                          <div className="flex items-center justify-between bg-navy-light/60 p-3.5 rounded-2xl border border-navy/15">
                            <div>
                              <span className="text-[10px] font-bold text-navy uppercase tracking-wider block">
                                Détail de Répartition du Bouquet
                              </span>
                            ...
                            <button
                              ...
                              className="px-3 py-1.5 rounded-xl bg-background border border-border text-xs font-semibold text-navy hover:bg-background-secondary transition-colors cursor-pointer"
                            >
                              Fermer
                            </button>
                          </div>
