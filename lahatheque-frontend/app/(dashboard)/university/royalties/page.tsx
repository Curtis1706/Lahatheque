...
export default function UniversityRoyaltiesPage() {
    ...
    try {
      ...
      await generateOfficialPdf({
        ...
        tableRows: filteredUnitSales.map((s) => [
          s.transaction_ref,
          s.date,
          `${s.book_title} (${s.format === "paper" ? "Papier" : (s.format === "audio" ? "Audio" : "Numérique")})`,
          s.quantity,
  ...
  const unitSalesColumns: DataTableColumn<UniversityUnitSaleRoyalty>[] = [
    ...
    },
    {
      ...
      cell: (row) => {
        if (row.format === "paper") {
          return (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-500/10 text-amber-800 border border-amber-500/20 shadow-2xs">
        ...
        if (row.format === "audio") {
          return (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold bg-gold/15 text-navy border border-gold/30 shadow-2xs">
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 w-full space-y-8 max-w-7xl mx-auto">
      ...
      <div className="border-b border-border pb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-foreground-muted mt-1">
            Relevé analytique en temps réel des droits perçus sur les ventes unitaires papier, numériques, audio et la quote-part des abonnements bouquets.
          </p>
        </div>
      </div>
      ...
      {/* 4 Cartes KPIs de Synthèse Analytique (Papier, Numérique, Audio & Bouquets) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-background border border-border space-y-3 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-navy uppercase tracking-wider">
              Ventes Audio à l&apos;Unité
            </span>
            <div className="p-2 rounded-xl bg-gold/15 border border-gold/30 text-gold-dark">
              <Headphones className="w-4 h-4 text-gold" />
            </div>
          </div>
          <div>
            <p className="text-2xl font-serif font-bold text-navy">
              {(data.totals_summary.audio_royalties_total ?? 0).toLocaleString("fr-FR")}{" "}
              <span className="text-xs font-sans text-gold">{data.currency}</span>
            </p>
            <p className="text-[11px] text-foreground-muted mt-1">
              Redevance nette perçue sur {data.totals_summary.audio_sales_count ?? 0} livres audio
            </p>
          </div>
          <div className="pt-2 border-t border-border flex items-center justify-between text-[11px] text-foreground-muted">
            <span>CA Brut Ventes :</span>
            <span className="font-mono font-semibold text-navy">
              {(data.totals_summary.audio_gross_total ?? 0).toLocaleString("fr-FR")} {data.currency}
            </span>
          </div>
        </div>

        {/* KPI 4 : Quotes-Parts Abonnements Bouquets (Prorata) */}
        <div className="p-5 rounded-2xl bg-background border border-border space-y-3 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-navy uppercase tracking-wider">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border pb-3">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>Ventes à l&apos;Unité (Papier, Numérique &amp; Audio)</span>
              ...
              <div className="flex flex-wrap items-center gap-2">
                <div className="flex items-center gap-1 bg-background border border-border rounded-xl px-2.5 py-1">
                  ...
                  >
                    ...
                    <option value="audio">Livre Audio uniquement</option>
                  </select>
                </div>
                ...
                {data.bouquet_royalties.map((bouquet) => {
                  return (
                    ...
                    >
                      ...
                      {/* Panneau de Répartition et Camembert d'Audience affiché automatiquement */}
                      <div className="mt-4 pt-4 border-t border-border space-y-4">
                        <div className="flex items-center justify-between bg-navy-light/60 p-3.5 rounded-2xl border border-navy/15">
                          <div>
                            <span className="text-[10px] font-bold text-navy uppercase tracking-wider block">
                              Répartition des Consultations & Redevances
                            </span>
                            <h4 className="font-serif text-sm sm:text-base font-bold text-navy">
                              {bouquet.bouquet_title}
                            </h4>
                          </div>
                          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-bold bg-gold/15 text-navy border border-gold/30">
                        ...
                        <BouquetDistributionContent
                          bouquet={{
                            ...
                            title: bouquet.bouquet_title,
                            annual_price: bouquet.bouquet_revenue_allocated,
