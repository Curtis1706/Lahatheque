...
- Q: Quels types d'articles et modes de règlement doivent être autorisés pour une commande comptoir externe sans compte utilisateur ? → A: Option A (Papier et/ou numérique autorisés avec e-mail obligatoire si format numérique, règlement immédiat en espèces ou Mobile Money direct avec statut « Payé » ou « En attente », et option de remise immédiate au comptoir).
- Q: Quel comportement d'interaction la nouvelle Combobox de recherche de client doit-elle adopter dans le formulaire ? → A: Option A (Champ interactif avec menu déroulant flottant instantané avec avatar, nom, rôle, téléphone, fermeture automatique à la sélection et carte récapitulative compacte avec bouton « Changer de client »).
...
- **FR-017**: Pour les commandes passées pour un client comptoir externe, le formulaire administrateur MUST autoriser les livres papier (avec option remise immédiate en boutique ou livraison) ainsi que les livres numériques/audio (en rendant alors l'e-mail obligatoire pour l'envoi des accès), avec choix du mode de règlement comptoir (espèces ou Mobile Money direct).
