...
2. **Un tunnel de commande e-commerce standardisé (`/cart`, `/checkout`)** permettant aux visiteurs non connectés d'alimenter un panier persistant localement, et de s'identifier au checkout via un panneau intégré à double onglet (Connexion directe ou Inscription rapide avec OTP, rôle `student`) sans jamais perdre leur panier.
3. **Une gestion multi-formats étanche** : persistance des coordonnées de livraison physique dans `PhysicalDelivery` avec alerte pour les gestionnaires stock, et déverrouillage instantané des formats numériques et audio dans la bibliothèque de l'espace client (`/student/books`).
...
- [x] **Principe I (Cadrage Métier & Lecture Intégrale Exhaustive)** : Modèles et vues Django inspectés à 100% (`apps/accounts/models.py`, `apps/communications/views.py`, `apps/commerce/models.py`, `apps/commerce/views.py`).
...
├── checklists/
│   └── requirements.md  # Checklist qualité des exigences (16/16)
...
│   └── commerce/
│       ├── views.py         # CreateOrderView, enregistrement PhysicalDelivery et ReadingProgress
│       └── services.py      # Déverrouillage d'accès et e-mail de confirmation
lahatheque-frontend/
├── app/
│   └── (public)/
│       ├── contact/
...
│       └── checkout/
│           └── page.tsx     # Tunnel e-commerce avec panneau identification Option A
├── components/
│   └── checkout/
