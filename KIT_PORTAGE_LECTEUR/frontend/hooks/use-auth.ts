...
import { getAuthToken, getRefreshToken, getActiveRole, clearAuthTokens } from "@/lib/auth-token";
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const setSessionCookie = (user: User | null, token?: string) => {
    if (user) {
      // Format plat — compatible avec les routes BFF admin qui lisent userData.role directement
      // NE PAS imbriquer dans { user: { role } } — les BFF attendent { role, token, active_roles }
        ...
        if (syncData?.user?.id) {
          setUser(prev => {
            if (!prev || JSON.stringify(prev) !== JSON.stringify(syncData.user)) {
              setSessionCookie(syncData.user, syncData.token || syncData.access)
          ...
          if (refreshRes.ok) {
            ...
            if (retryRes.ok) {
              ...
              if (retryData?.user?.id) {
                ...
                setSessionCookie(retryData.user, retryData.token || retryData.access)
  ...
  const loginWithToken = async (token: string, userData: User) => {
    ...
    // SÉCURITÉ : le token ne doit JAMAIS aller dans le localStorage (vulnérable au XSS).
    // En cas d'échec BFF, on loggue sans fallback — le cookie laha_access existant
    // ou un re-login résoudra la session au prochain refresh.
    try {
      const migrateRes = await fetch('/api/auth/session/', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (migrateRes.ok) {
        ...
        // S'assurer que le localStorage est bien vide
        clearAuthTokens()
      } else {
        // ⚠️ Pas de fallback localStorage — le token ne doit pas transiter en clair côté JS
        logger.warn('loginWithToken: migration BFF échouée. Session sera rétablie au prochain refresh.', { context: 'useAuth' })
        clearAuthTokens()
      }
    } catch (e) {
      // Erreur réseau : on ne stocke PAS en localStorage
      logger.warn('loginWithToken: erreur réseau BFF. Session sera rétablie au prochain refresh.', { context: 'useAuth' })
      clearAuthTokens()
    }
  }
