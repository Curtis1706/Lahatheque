...
function NotePin({ annotation, onDelete, activeTool }: { annotation: Annotation; onDelete: (id: string) => void; activeTool: ActiveTool }) {
  ...
  return (
    ...
    >
      <button
        className={cn(
          "w-7 h-7 rounded-full bg-[#121620] text-laha-gold shadow-[0_4px_12px_rgba(0,0,0,0.5)] border border-white/10 flex items-center justify-center transition-transform hover:scale-110",
          activeTool === 'erase' && "ring-2 ring-red-500 bg-red-500/20 text-red-400"
        )}
      ...
      <AnimatePresence>
        {showTooltip && activeTool !== 'erase' && (
          <motion.div
            ...
            className="absolute left-10 top-0 bg-[#0B0F19]/95 backdrop-blur-xl border border-white/10 rounded-2xl p-4 w-64 shadow-[0_20px_40px_rgba(0,0,0,0.5)] z-50 origin-top-left"
          >
            <div className="flex items-center justify-between mb-3 border-b border-white/10 pb-2">
              <p className="text-[10px] font-black text-laha-gold uppercase tracking-widest flex items-center gap-2">
                <StickyNote size={12} /> Note sauvegardée
              </p>
              <button onClick={() => setShowTooltip(false)} className="text-white/40 hover:text-white transition-colors">
                <X size={14} />
              </button>
            </div>
            <p className="text-sm text-white/90 leading-relaxed italic">{annotation.content}</p>
            <button
              onClick={() => onDelete(annotation.id)}
              className="mt-4 pt-3 border-t border-white/10 w-full text-[10px] font-bold uppercase text-red-400/80 hover:text-red-400 flex items-center justify-center gap-1.5 transition-colors"
            >
