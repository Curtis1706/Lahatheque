...
4. **Appelle `get_component`** uniquement sur le(s) candidat(s) présélectionné(s) pour lire le code réel. Si la réponse indique `locked=true` (quota atteint), bascule automatiquement sur les serveurs suivants (`21st-2`, `21st-3`, ..., `21st-8`). Déclenche l'alerte quota/protocole copier-coller uniquement si l'ensemble des 8 instances est totalement épuisé.
