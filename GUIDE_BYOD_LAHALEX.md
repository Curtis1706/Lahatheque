...
```
 ...
 └──────────────────────────────────────────────────────────────────────────────────────────────────┘
   ...
   3. Le backend LAHALEX contacte l'API LAHAThèque (https://lahatheque.vercel.app) en 2 sous-étapes :
        a) Obtention du token OAuth2 partenaire (si expiré).
        b) Création de la session avec l'URL du PDF et l'identité du juriste.
   4. LAHAThèque génère une URL de lecture éphémère (ex: https://lahatheque.vercel.app/read/eyJh...)
...
Ces identifiants sont déjà actifs en base de données sur **`https://lahatheque.vercel.app`** :
...
| **URL de Base de l'API** | `https://lahatheque.vercel.app` | Serveur API LAHAThèque en production |
...
#### Requête HTTP
```http
POST https://lahatheque.vercel.app/api/v1/oauth2/token/
...
#### Requête HTTP
```http
POST https://lahatheque.vercel.app/api/v1/reader/sessions/
...
```json
{
  ...
  "data": {
    ...
    "reader_url": "https://lahatheque.vercel.app/read/eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.session_token_unique",
...
* Lorsque le juriste arrive sur `https://lahatheque.vercel.app/read/...`, le lecteur démarre en mode 3D immersif ou défilement vertical.
...
{
    private string $baseUrl = 'https://lahatheque.vercel.app';
...
const LAHATHÈQUE_BASE = 'https://lahatheque.vercel.app';
...
async function getAccessToken(): Promise<string> {
  ...
  const res = await axios.post(`${LAHATHÈQUE_BASE}/api/v1/oauth2/token/`, params, {
...
export async function createLahalexReaderSession(data: ReaderRequest): Promise<string> {
...
LAHATHÈQUE_BASE = "https://lahatheque.vercel.app"
...
* **Tableau de Bord Administrateur :** `https://lahatheque.vercel.app/admin/api`
* **Supervision des Sessions en Direct :** `https://lahatheque.vercel.app/admin/api/sessions`
* **Logs & Webhooks :** `https://lahatheque.vercel.app/admin/api/logs`
